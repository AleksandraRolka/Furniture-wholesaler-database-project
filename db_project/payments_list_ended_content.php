...
if($_SESSION['role']=='magazynier')
{
	...
	echo "<p class='ListHeader'><u>Lista pomyślnie zakończonych płatności</u></p>";
	...
	$result = pg_query($db_conn, "SELECT * FROM platnosci_lista WHERE id_magazyn=$id_wh AND status_platnosci='zrealizowana';");
	$rows = pg_num_rows($result);
	if ( $rows > 0 ) 
	{
		$i = 0;
