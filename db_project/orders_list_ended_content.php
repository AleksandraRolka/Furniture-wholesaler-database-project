...
if($_SESSION['role']=='magazynier')
{
	...
	echo "<p class='ListHeader'><u>Lista zrealizowanych zamówień</u></p>";
	...
	$result = pg_query($db_conn, "SELECT * FROM zamowienia_lista WHERE id_magazyn=$id_wh AND status='zakończone';");
	$rows = pg_num_rows($result);
	if ( $rows > 0 ) 
	{
		$i = 0;
